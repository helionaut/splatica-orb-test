...
def safe_extract(archive: tarfile.TarFile, destination: Path) -> None:
    ...
    for member in archive.getmembers():
        ...
        if not is_within(member_path, resolved_destination):
            raise ValueError(f"Archive member escaped extraction root: {member.name}")
...
def main() -> int:
    ...
    if public_tum_vi_dataset_is_ready(resolved):
    ...
    staging_root = resolved.dataset_root.parent / f".{resolved.dataset_root.name}.extracting"
    if staging_root.exists():
        shutil.rmtree(staging_root)
    staging_root.mkdir(parents=True, exist_ok=True)

    try:
        with tarfile.open(resolved.archive, mode="r:") as archive:
