...
- `make bootstrap-local-opencv`: extract the Ubuntu OpenCV 4 dev/runtime package set into `build/local-tools/opencv-root/` so the repo can satisfy `find_package(OpenCV 4.4)` and `pkg-config opencv4` without a system-wide install.
...
- `./scripts/bootstrap_local_opencv.sh`: extract the Ubuntu OpenCV 4 dev/runtime package set into `build/local-tools/opencv-root/`. `./scripts/build_orbslam3_baseline.sh` and `make monocular-prereqs` will reuse that prefix automatically if the host does not already provide OpenCV.
...
- `./scripts/build_orbslam3_baseline.sh`: run the pinned ORB-SLAM3 native build sequence so `Examples/Monocular/mono_tum_vi` exists. The wrapper keeps the upstream component order, automatically reuses repo-local `cmake`, `Eigen3`, OpenCV, and Boost serialization fallbacks, and disables optional `Thirdparty/Sophus` tests/examples because they are not required for `mono_tum_vi` and fail under newer GCC releases when upstream `-Werror` is enabled. Pangolin still has to be provided either system-wide or through a manual local install under `build/local-tools/pangolin-root/usr/local/`; Ubuntu `noble` does not currently expose a `libpangolin-dev` apt package.
